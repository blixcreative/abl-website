...
export default async function AdminHomepagePage({ searchParams }: PageProps) {
  ...
  const homeModules = adminModules.filter((m) => 
    ["homeHero", "homePartner", "homeSolution", "homeMiniSlide", "homeProductCategories"].includes(m.key)
  );

  // Mặc định chọn thành phần đầu tiên nếu không có tab trong URL
  const activeModule = homeModules.find((m) => m.key === tab) || homeModules[0] || null;
    ...
    <main className="min-h-full bg-gray-100 text-slate-900">
      <header className="flex items-center px-8 pt-8">
        <div className="flex items-center gap-4">
          ...
          <span className="font-semibold text-slate-800">Trang chủ</span>
        </div>
      </header>
          ...
          <nav className="flex flex-col gap-2 rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-100">
            {homeModules.map((module) => {
              ...
              return (
                <Link
                  key={module.key}
                  href={`/admin/homepage?tab=${module.key}`}
