...
export async function upsertAboutMiniSlide(formData: FormData) {
  const supabase = await createSupabaseServerClient();
  if (!supabase) throw new Error("Unauthorized");
  ...
  const title = formData.get("title")?.toString() || "";
  const description = formData.get("description")?.toString() || "";
  const sort_order = parseInt(formData.get("sort_order")?.toString() || "0");
  
  const payload: any = { title, description, sort_order };
  const imageBase64 = await normalizeFile(formData.get("image_url"));
  if (imageBase64) payload.image_url = imageBase64;

  if (id) {
    await supabase.from("cms_about_mini_slide").update(payload).eq("id", id);
  } else {
    payload.status = "published";
    await supabase.from("cms_about_mini_slide").insert(payload);
  }
  revalidatePath("/admin/about");
}

export async function deleteAboutMiniSlide(formData: FormData) {
  const supabase = await createSupabaseServerClient();
  if (!supabase) throw new Error("Unauthorized");
  const id = formData.get("id")?.toString();
  if (id) await supabase.from("cms_about_mini_slide").delete().eq("id", id);
  revalidatePath("/admin/about");
}

export async function upsertAboutPartner(formData: FormData) {
  const supabase = await createSupabaseServerClient();
  if (!supabase) throw new Error("Unauthorized");
  ...
  const imageBase64 = await normalizeFile(formData.get("image_file"));
  if (imageBase64) payload.image_file = imageBase64;

  if (id) {
    await supabase.from("cms_about_partner").update(payload).eq("id", id);
  } else {
    payload.status = "published";
    await supabase.from("cms_about_partner").insert(payload);
  }
  revalidatePath("/admin/about");
}

export async function deleteAboutPartner(formData: FormData) {
  const supabase = await createSupabaseServerClient();
  if (!supabase) throw new Error("Unauthorized");
  const id = formData.get("id")?.toString();
  if (id) await supabase.from("cms_about_partner").delete().eq("id", id);
  revalidatePath("/admin/about");
}

export async function upsertAboutAward(formData: FormData) {
  const supabase = await createSupabaseServerClient();
  if (!supabase) throw new Error("Unauthorized");
  ...
  if (id) {
    await supabase.from("cms_about_award").update(payload).eq("id", id);
  } else {
    payload.status = "published";
    await supabase.from("cms_about_award").insert(payload);
  }
  revalidatePath("/admin/about");
}

export async function deleteAboutAward(formData: FormData) {
  const supabase = await createSupabaseServerClient();
  if (!supabase) throw new Error("Unauthorized");
  const id = formData.get("id")?.toString();
  if (id) await supabase.from("cms_about_award").delete().eq("id", id);
  revalidatePath("/admin/about");
}